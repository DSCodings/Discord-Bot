...
module.exports.run = async (bot, message, args) => {
    ...
    if (!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send("You don't have the permission for that.");

    // User opvragen en nakijken als je wel een user meegeeft.
    var user = message.guild.member(message.mentions.users.first());

    if (!user) return message.channel.send("<prefix>tempban user time reason");
...
module.exports.help = {
    name: "tempban",
    description: "Band someone for a while (only for administrator)"
}